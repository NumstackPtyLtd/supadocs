      ...
      <header className="sd-header">
        <div className="sd-header-inner">
          ...
          <div className="sd-header-right">
            {config.versions?.length > 1 && <VersionDropdown versions={config.versions} />}
...
function VersionDropdown({ versions }: { versions: any[] }) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  const location = window.location.pathname;

  // Find current version based on URL
  const current = versions.find((v: any) => {
    if (v.default) return !versions.some((o: any) => !o.default && location.startsWith(o.path));
    return location.startsWith(v.path);
  }) || versions.find((v: any) => v.default) || versions[0];
  ...
  return (
    <div className="sd-version-dropdown" ref={ref}>
      <button className="sd-version-btn" onClick={() => setOpen(!open)} aria-label="Select version">
        {current.label}
      ...
      {open && (
        <div className="sd-version-menu">
          {versions.map((v: any) => (
            <a
              key={v.path}
              href={v.path}
              className={`sd-version-item ${v.label === current.label ? 'sd-version-active' : ''}`}
              onClick={() => setOpen(false)}
            >
              {v.label}
              {v.default && <span className="sd-version-badge">latest</span>}
            </a>
          ))}
        </div>
      )}
